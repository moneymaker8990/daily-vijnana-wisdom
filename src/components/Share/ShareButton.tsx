...
export function ShareButton({ text, title, source, className = '', variant = 'icon', dayNumber }: ShareButtonProps) {
  ...
  if (variant === 'full') {
    return (
      <div ref={menuRef} className={`relative ${className}`}>
        ...
        {showMenu && (
          <div className="absolute bottom-full mb-2 left-0 bg-slate-800/95 backdrop-blur-xl border border-white/10 rounded-xl p-2 shadow-xl animate-fadeIn z-50 min-w-[140px]" role="menu">
            <button
              onClick={handleCopy}
  ...
  return (
    <div ref={menuRef} className={`relative inline-block ${className}`}>
      ...
      {showMenu && (
        <div className="absolute bottom-full mb-2 right-0 bg-slate-800/95 backdrop-blur-xl border border-white/10 rounded-xl p-2 shadow-xl animate-fadeIn z-50 min-w-[140px]" role="menu">
          <button
            onClick={handleCopy}
            className="flex items-center gap-3 w-full px-3 py-2 text-white/70 hover:text-white hover:bg-white/10 rounded-lg text-sm transition-colors"
            role="menuitem"
